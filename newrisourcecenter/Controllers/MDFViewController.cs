...
namespace newrisourcecenter.Models
{
    public class MDFViewController : Controller
    {
        ...
        public async Task<string> GetApprovers(long comp_ID)
        {
            ...
            var company = db.partnerCompanyViewModels.Where(a => a.comp_ID == comp_ID).FirstOrDefault();
            if(company != null && !string.IsNullOrEmpty(company.approver_emails))
            {
                approvers = company.approver_emails.Split(',').ToList();
            }
            else if (companies.Contains(comp_ID.ToString()) || (userLocation != null && companyLocations.Contains(comp_ID + ":" + userLocation.loc_ID)))
            {
        ...
        public Dictionary<string,string> getRCM()
        {
            ...
            var other_rcms = dbEntity.mdf_main
                .Where(m => !string.IsNullOrEmpty(m.rcm))
                .GroupJoin(dbEntity.RCMContacts, m => m.rcm, rc => rc.email, (m, rc) => new { m, rc })
                .SelectMany(x => x.rc.DefaultIfEmpty(), (x, rc) => new { x.m, rc })
                .Where(x => x.rc == null)
                .GroupJoin(dbEntity.usr_user, x => x.m.rcm, u => u.usr_email, (x, u) => new { x.m, u })
                .SelectMany(x => x.u.DefaultIfEmpty(), (x, u) => new
                {
                    email = x.m.rcm,
                    name = u != null ? (u.usr_fName + " " + u.usr_lName).Trim() : null
                });
            foreach(var row in other_rcms)
            {
               if(!dict.ContainsKey(row.email))
               {
                    dict[row.email] = row.name;
               }
            }
            return dict;
        }
        ...
        public async Task<ActionResult> RequestReportForm(string year=null)
        {
            ...
            if (string.IsNullOrEmpty(year))
            {
                ...
                mdf_to_excel.Columns.Add("Activity Description", typeof(string));
